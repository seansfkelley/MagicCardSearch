...
- **FakeSearchBarButtonView**: A persistent bottom button that opens the search sheet
...
└── Tab: Search (visually distinct button)
    └── SearchTabView (uses .searchable, contains search interface + results)
```
...
- `SearchTabView.swift` - Search tab with `.searchable` modifier and integrated results

### 3.3 Search Suggestions Approach

Use the `.searchSuggestions` modifier with the existing autocomplete behavior, including custom subviews and swipe actions.
...
Stub placeholder view for now.

### 5.3 SearchTabView

Combines current search and results functionality, with `.searchable` modifier applied to this view:

```
SearchTabView
├── .searchable modifier with .searchSuggestions for autocomplete
├── NavigationStack
│   ├── When no results / search not active:
│   │   └── Default state content:
│   │       - Pinned searches (from PinnedSearchEntry)
│   │       - Recent searches (from SearchHistoryEntry)
│   │       - Example searches
│   ├── When searching / has results:
│   │   └── SearchResultsGridView
```

Autocomplete should function as close as possible to how it currently functions, including:
- Pin/unpin functionality on search history items
- Swipe-to-delete on history items
- Custom row views for different suggestion types
- Real-time autocomplete suggestions based on current input
...
4. Create `SearchTabView` with `.searchable` modifier, combining search + results
