...
struct HistorySuggestionProviderTests {
    ...
    private func loadFilters(_ filter: [SearchFilter]) {
        for f in filter {
            provider.recordUsage(of: f)
        }
    }
    ...
    func singleFilter() {
        let filter = SearchFilter.basic(.keyValue("color", .equal, "red"))
        provider.recordUsage(of: filter)
        let suggestions = provider.getSuggestions(for: "", excluding: Set(), limit: 10)
        
        #expect(suggestions = [HistorySuggestion(filter: filter, isPinned: false, matchRange: nil)])
    }
