...
func getNameSuggestions(for partial: PartialFilterTerm, cardNames: [String]?, limit: Int) -> [NameSuggestion] {
    guard limit > 0, let cardNames else {
        return []
    }
    ...
    //   let results = (cardNames ?? []).compactMap { candidate in
    let start = ContinuousClock().now
    let results: [(String, ScoredMatch)] = cardNames.compactMap { candidate -> (String, ScoredMatch)? in
    ...
    logger.info("Fuzzy match over \(cardNames.count) names took \(elapsed)")
