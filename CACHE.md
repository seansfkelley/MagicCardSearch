...
class WeakMemoryStorage<K: Hashable, V>: Cache<K, V> {
    init(expiry: Expiry = .never, countLimit: Int? = nil, clock: some Clock = ContinuousClock())
}
class StrongMemoryStorage<K: Hashable, V>: Cache<K, V> {
    init(expiry: Expiry = .never, countLimit: Int? = nil, gcInterval: Duration? = .seconds(300), clock: some Clock = ContinuousClock())
}
class DiskStorage<K: Codable & Hashable, V>: Cache<K, V> {
    init?(name: String, expiry: Expiry = .never, encode: (V) throws -> Data, decode: (Data) throws -> V, countLimit: Int? = nil, gcInterval: Duration? = .seconds(300), clock: some Clock = ContinuousClock())
}
...
extension DiskStorage where V: Codable {
    convenience init?(name: String, expiry: Expiry = .never, countLimit: Int? = nil, gcInterval: Duration? = .seconds(300), clock: some Clock = ContinuousClock())
}
...
The `expiry` parameter on `get(_:expiry:through:)` and `put(_:_:expiry:)` overrides the storage's default expiry for that specific entry. Passing `nil` uses the storage's configured default.
...
`NSCache` does not expose entry metadata, so `WeakMemoryStorage` maintains a `[K: Capsule]` dictionary (protected by an `NSLock`) as the authoritative store for both values and expiry dates, where `Capsule` holds a `value: V` and an `expiryDate: Date?` (`nil` = never expires). NSCache is used solely as an eviction signal — when the OS evicts an entry, the next `get` misses NSCache, removes the corresponding capsule from the dictionary, and returns `nil`. This is an accepted limitation; stale capsules are cleaned up on next access and do not affect correctness.
...
**Read behavior:** Check delegates in order via `getWithMetadata`. On a hit in delegate N, backfill all preceding delegates (0..<N) with the found value. The backfill expiry is `min(remaining TTL of found entry, delegate's own default expiry)` — a nearly-expired entry is not backfilled with a longer TTL than it has remaining. Remaining TTL is derived from `CacheEntry.expiryDate`; if `nil` (never expires), the delegate's own default expiry applies.
...
**App backgrounding:** When iOS suspends the app, Swift `Task`s are paused at their current suspension point — typically mid-sleep for a GC task. The injected clock's elapsed time continues to accumulate during the suspension. On foreground resume, if the configured `gcInterval` has elapsed, the GC sweep fires immediately. No special background task registration is needed; GC is opportunistic and only runs while the app is active.
...
- **`DiskStorage`:** Runs `DELETE FROM entries WHERE expiry IS NOT NULL AND expiry < ?`, passing the current Unix timestamp from the injected clock. This keeps GC behavior consistent with the injected clock rather than the real wall clock, which matters for testing.
...
- **`WeakMemoryStorage`:** `NSCache` is internally thread-safe. The capsule dictionary is protected by an `NSLock`.
...
- GC: use `Expiry.date` in the past, trigger a GC sweep directly (expose an internal `removeExpiredEntries()` method for testing), verify expired entries are gone
